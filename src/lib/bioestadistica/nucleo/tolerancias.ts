 ...
 *
 * Vive en la biblioteca pura (no en `tests/`) porque tiene dos consumidores: las
 * pruebas `node --test` con los fixtures commiteados y el panel «Verificar con
 * R» del navegador, que compara el resultado de TypeScript con el JSON que
 * imprime webR usando exactamente el mismo perfil (H4). `tests/bioestadistica/
 * tolerancias.ts` reexporta este módulo para las pruebas.
 *
 ...
 */
import type { Entradas } from './tipos.ts';
import type { PerfilTolerancia, Tolerancia } from './comparar.ts';
...
/**
 * Perfil que corresponde a unas entradas concretas de una calculadora: el del
 * slug salvo que un caso del fixture usaría otro. Es la misma regla que fija
 * `tol` en `tests/bioestadistica/casos/<slug>.json`, escrita una sola vez para
 * que el panel «Verificar con R» juzgue con el criterio de las pruebas.
 *
 * - `prueba-diagnostica-2x2` con Clopper-Pearson o Jeffreys invierte la beta
 *   incompleta (`qbeta`): las seis proporciones pasan a `cuantil`
 *   (`prueba-diagnostica-2x2-beta`).
 *
 * @throws {Error} si el slug no tiene perfil: una calculadora nueva debe
 *   declararlo antes de publicarse (lo vigila `tolerancias.test.ts`).
 */
export function perfilPara(slug: string, entradas: Entradas): PerfilTolerancia {
  let clave = slug;
  if (slug === 'prueba-diagnostica-2x2' && (entradas.metodo === 'clopper-pearson' || entradas.metodo === 'jeffreys')) {
    clave = 'prueba-diagnostica-2x2-beta';
  }
  const perfil = TOL[clave];
  if (!perfil) throw new Error(`tolerancias: la calculadora «${slug}» no tiene perfil en TOL`);
  return perfil;
}
