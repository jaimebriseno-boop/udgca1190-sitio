...
vp <- 68; fp <- 0; fn <- 12; vn <- 120   # rows = test result (+, -), columns = reference standard (diseased, not diseased)
nivel  <- 0.95
metodo <- "wilson"   # CI method for proportions: "wilson", "clopper-pearson", "agresti-coull", "jeffreys" or "wald"
corr   <- 0.5     # 0.5 = Haldane-Anscombe correction for the ratios (LR, DOR) when a cell is 0; 0 = none
n <- vp + fp + fn + vn
z <- qnorm(1 - (1 - nivel) / 2)
