...
de_dif      <- 0        # SD of the DIFFERENCES; 0 = not captured, then it is derived below
sigma       <- 1.5         # SD of each measurement; only used when de_dif is not given (0 = not captured)
rho         <- 0.9           # correlation between the two measurements; only used together with sigma
alfa        <- 0.05
