...
- `pegado.ts` (`parsearPegado(texto) → { columnas, filas, avisos }`): detecta separador (tab > `;` > `,`), fila de encabezado (primera fila no numérica), `NA`/vacío/`.` como faltante, coma decimal cuando el separador no es coma y el token cumple `^-?\d+,\d+$`, comillas de hoja de cálculo, `\r\n`. Probado en `pegado.test.ts` con pegados reales de Excel y Google Sheets.
